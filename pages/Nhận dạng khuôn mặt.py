...
svc = joblib.load(r'pages\model\svc.pkl')
mydict = ['BanKiet', 'BanNghia', 'BanNguyen',
          'BanThanh', 'SangSang', 'ThayDuc']
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if faces[1] is not None:
            ...
            # location_x = int(faces[1][0][0])
            # location_y = int(faces[1][0][1])
            # # location_w = int(faces[1][0][2])
            # # location_h = int(faces[1][0][3])
            # cv.putText(frame, result, (location_x, location_y-10),
            #            cv.FONT_HERSHEY_SIMPLEX, 1.11, (0, 255, 0), 2)
            for face in faces[1]:
                face_align = recognizer.alignCrop(frame, face)
                face_feature = recognizer.feature(face_align)
                test_predict = svc.predict(face_feature)
                result = mydict[test_predict[0]]
                location_x = int(face[0])
                location_y = int(face[1])
                cv.putText(frame, result, (location_x, location_y-10),
                           cv.FONT_HERSHEY_SIMPLEX, 1.11, (0, 255, 0), 2)
